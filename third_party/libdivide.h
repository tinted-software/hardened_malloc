...
static LIBDIVIDE_INLINE struct libdivide_u16_t
libdivide_internal_u16_gen(uint16_t d, int branchfree) {
  if (d == 0) {
...
uint16_t
libdivide_u16_branchfree_do(uint16_t numer,
                            const struct libdivide_u16_branchfree_t *denom) {
...
static LIBDIVIDE_INLINE struct libdivide_u32_t
...
static LIBDIVIDE_INLINE struct libdivide_u64_t
libdivide_internal_u64_gen(uint64_t d, int branchfree) {
  ...
  if ((d & (d - 1)) == 0) {
    ...
    proposed_m =
        libdivide_128_div_64_to_64((uint64_t)1 << floor_log_2_d, 0, d, &rem);
...
static LIBDIVIDE_INLINE struct libdivide_s16_t
libdivide_internal_s16_gen(int16_t d, int branchfree) {
  if (d == 0) {
    LIBDIVIDE_ERROR("divider must be != 0");
  }
  ...
  if ((absD & (absD - 1)) == 0) {
    ...
    uint16_t rem, proposed_m;
    proposed_m = libdivide_32_div_16_to_16((uint16_t)1 << (floor_log_2_d - 1),
                                           0, absD, &rem);
...
int16_t
libdivide_s16_branchfree_do(int16_t numer,
                            const struct libdivide_s16_branchfree_t *denom) {
  uint8_t more = denom->more;
  uint8_t shift = more & LIBDIVIDE_16_SHIFT_MASK;
  // must be arithmetic shift and then sign extend
...
static LIBDIVIDE_INLINE struct libdivide_s32_t
libdivide_internal_s32_gen(int32_t d, int branchfree) {
  ...
  if ((absD & (absD - 1)) == 0) {
    ...
    // is 0 and the high word is floor_log_2_d - 1
    uint32_t rem, proposed_m;
    proposed_m = libdivide_64_div_32_to_32((uint32_t)1 << (floor_log_2_d - 1),
                                           0, absD, &rem);
...
libdivide_internal_s64_gen(int64_t d, int branchfree) {
  ...
  if ((absD & (absD - 1)) == 0) {
    ...
    proposed_m = libdivide_128_div_64_to_64((uint64_t)1 << (floor_log_2_d - 1),
                                            0, absD, &rem);
...
static LIBDIVIDE_INLINE uint32x4_t libdivide_mullhi_u32_vec128(uint32x4_t a,
  ...
  uint32x4_t w1 =
      vreinterpretq_u32_u64(vmull_n_u32(vget_low_u32(a), b)); // [_, x0, _, x1]
  uint32x4_t w2 =
...
static LIBDIVIDE_INLINE int32x4_t libdivide_mullhi_s32_vec128(int32x4_t a,
                                                              int32_t b) {
  int32x4_t w1 =
      vreinterpretq_s32_s64(vmull_n_s32(vget_low_s32(a), b)); // [_, x0, _, x1]
...
int32x4_t libdivide_s32_do_vec128(int32x4_t numers,
  ...
  if (!denom->magic) {
    ...
    int32x4_t q =
        vaddq_s32(numers, vandq_s32(vshrq_n_s32(numers, 31), roundToZeroTweak));
    ...
    q = vaddq_s32(q, vreinterpretq_s32_u32(vshrq_n_u32(vreinterpretq_u32_s32(q),
                                                       31))); // q += (q < 0)
    return q;
  }
}
...
int64x2_t libdivide_s64_do_vec128(int64x2_t numers,
  ...
  if (magic == 0) { // shift path
    ...
    int64x2_t q = vaddq_s64(
        numers, vandq_s64(libdivide_s64_signbits(numers), roundToZeroTweak));
    ...
    q = vaddq_s64(q, vreinterpretq_s64_u64(vshrq_n_u64(vreinterpretq_u64_s64(q),
                                                       63))); // q += (q < 0)
    return q;
  }
}
...
__m512i libdivide_s32_do_vec512(__m512i numers,
  ...
  if (!denom->magic) {
    ...
    // q = numer + ((numer >> 31) & roundToZeroTweak);
    __m512i q =
        _mm512_add_epi32(numers, _mm512_and_si512(_mm512_srai_epi32(numers, 31),
                                                  roundToZeroTweak));
    ...
    __m512i q =
        libdivide_mullhi_s32_vec512(numers, _mm512_set1_epi32(denom->magic));
    if (more & LIBDIVIDE_ADD_MARKER) {
      // must be arithmetic shift
      __m512i sign = _mm512_set1_epi32((int8_t)more >> 7);
      // q += ((numer ^ sign) - sign);
...
__m256i libdivide_s32_do_vec256(__m256i numers,
  ...
  if (!denom->magic) {
    ...
    // q = numer + ((numer >> 31) & roundToZeroTweak);
    __m256i q =
        _mm256_add_epi32(numers, _mm256_and_si256(_mm256_srai_epi32(numers, 31),
                                                  roundToZeroTweak));
    ...
    __m256i q =
        libdivide_mullhi_s32_vec256(numers, _mm256_set1_epi32(denom->magic));
    if (more & LIBDIVIDE_ADD_MARKER) {
      // must be arithmetic shift
      __m256i sign = _mm256_set1_epi32((int8_t)more >> 7);
      // q += ((numer ^ sign) - sign);
...
__m128i libdivide_s64_do_vec128(__m128i numers,
  ...
  if (magic == 0) { // shift path
    ...
    __m128i q = _mm_add_epi64(
        numers,
        _mm_and_si128(libdivide_s64_signbits_vec128(numers), roundToZeroTweak));
...
template <> struct NeonVecFor<uint16_t> {
  typedef uint16x8_t type;
};

template <> struct NeonVecFor<int16_t> {
  typedef int16x8_t type;
};

template <> struct NeonVecFor<uint32_t> {
  typedef uint32x4_t type;
};

template <> struct NeonVecFor<int32_t> {
  typedef int32x4_t type;
};

template <> struct NeonVecFor<uint64_t> {
  typedef uint64x2_t type;
};

template <> struct NeonVecFor<int64_t> {
...
template <typename _IntT, Branching ALGO> struct dispatcher {};

template <> struct dispatcher<int16_t, BRANCHFULL> {
  DISPATCHER_GEN(int16_t, s16)
};
template <> struct dispatcher<int16_t, BRANCHFREE> {
  DISPATCHER_GEN(int16_t, s16_branchfree)
};
template <> struct dispatcher<uint16_t, BRANCHFULL> {
  DISPATCHER_GEN(uint16_t, u16)
};
template <> struct dispatcher<uint16_t, BRANCHFREE> {
  DISPATCHER_GEN(uint16_t, u16_branchfree)
};
template <> struct dispatcher<int32_t, BRANCHFULL> {
  DISPATCHER_GEN(int32_t, s32)
};
template <> struct dispatcher<int32_t, BRANCHFREE> {
  DISPATCHER_GEN(int32_t, s32_branchfree)
};
template <> struct dispatcher<uint32_t, BRANCHFULL> {
  DISPATCHER_GEN(uint32_t, u32)
};
template <> struct dispatcher<uint32_t, BRANCHFREE> {
  DISPATCHER_GEN(uint32_t, u32_branchfree)
};
template <> struct dispatcher<int64_t, BRANCHFULL> {
  DISPATCHER_GEN(int64_t, s64)
};
template <> struct dispatcher<int64_t, BRANCHFREE> {
  DISPATCHER_GEN(int64_t, s64_branchfree)
};
template <> struct dispatcher<uint64_t, BRANCHFULL> {
  DISPATCHER_GEN(uint64_t, u64)
};
template <> struct dispatcher<uint64_t, BRANCHFREE> {
...
public:
  ...
  // Vector variants treat the input as packed integer values with the same type
  // as the divider (e.g. s32, u32, s64, u64) and divides each of them by the
  // divider, returning the packed quotients.
#if defined(LIBDIVIDE_SSE2)
...
#endif
#if defined(LIBDIVIDE_NEON)
  LIBDIVIDE_INLINE typename NeonVecFor<T>::type
  divide(typename NeonVecFor<T>::type n) const {
    return div.divide(n);
  }
#endif
...
template <typename T, Branching ALGO>
LIBDIVIDE_INLINE typename NeonVecFor<T>::type
operator/(typename NeonVecFor<T>::type n, const divider<T, ALGO> &div) {
  return div.divide(n);
}
