...
#define SLAB_QUARANTINE                                                        \
  (H_MALLOC_SLAB_QUARANTINE_RANDOM_LENGTH > 0 ||                               \
   H_MALLOC_SLAB_QUARANTINE_QUEUE_LENGTH > 0)
#define REGION_QUARANTINE                                                      \
  (H_MALLOC_REGION_QUARANTINE_RANDOM_LENGTH > 0 ||                             \
   H_MALLOC_REGION_QUARANTINE_QUEUE_LENGTH > 0)
#define MREMAP_MOVE_THRESHOLD ((size_t)32 * 1024 * 1024)

static_assert(sizeof(void *) == 8, "64-bit only");

static_assert(!H_MALLOC_WRITE_AFTER_FREE_CHECK || H_MALLOC_ZERO_ON_FREE,
              "WRITE_AFTER_FREE_CHECK depends on ZERO_ON_FREE");

static_assert(H_MALLOC_SLAB_QUARANTINE_RANDOM_LENGTH >= 0 &&
                  H_MALLOC_SLAB_QUARANTINE_RANDOM_LENGTH <= 65536,
              "invalid slab quarantine random length");
static_assert(H_MALLOC_SLAB_QUARANTINE_QUEUE_LENGTH >= 0 &&
                  H_MALLOC_SLAB_QUARANTINE_QUEUE_LENGTH <= 65536,
              "invalid slab quarantine queue length");
static_assert(H_MALLOC_REGION_QUARANTINE_RANDOM_LENGTH >= 0 &&
                  H_MALLOC_REGION_QUARANTINE_RANDOM_LENGTH <= 65536,
              "invalid region quarantine random length");
static_assert(H_MALLOC_REGION_QUARANTINE_QUEUE_LENGTH >= 0 &&
                  H_MALLOC_REGION_QUARANTINE_QUEUE_LENGTH <= 65536,
              "invalid region quarantine queue length");
static_assert(H_MALLOC_FREE_SLABS_QUARANTINE_RANDOM_LENGTH >= 0 &&
                  H_MALLOC_FREE_SLABS_QUARANTINE_RANDOM_LENGTH <= 65536,
              "invalid free slabs quarantine random length");

static_assert(H_MALLOC_GUARD_SLABS_INTERVAL >= 1,
              "invalid guard slabs interval (minimum 1)");
static_assert(H_MALLOC_GUARD_SIZE_DIVISOR >= 1,
              "invalid guard size divisor (minimum 1)");
static_assert(H_MALLOC_CLASS_REGION_SIZE >= 1048576,
              "invalid class region size (minimum 1048576)");
static_assert(H_MALLOC_CLASS_REGION_SIZE <= 1099511627776,
              "invalid class region size (maximum 1099511627776)");
static_assert(H_MALLOC_REGION_QUARANTINE_SKIP_THRESHOLD >= 0,
              "invalid region quarantine skip threshold (minimum 0)");
static_assert(MREMAP_MOVE_THRESHOLD >=
                  H_MALLOC_REGION_QUARANTINE_SKIP_THRESHOLD,
              "mremap move threshold must be above region quarantine limit");

// either sizeof(u64) or 0
static const size_t canary_size = H_MALLOC_SLAB_CANARY ? sizeof(u64) : 0;

static_assert(H_MALLOC_N_ARENA >= 1, "must have at least 1 arena");
static_assert(H_MALLOC_N_ARENA <= 256,
              "maximum number of arenas is currently 256");
#define CACHELINE_SIZE 64

#if N_ARENA > 1
__attribute__((tls_model(
    "initial-exec"))) static _Thread_local unsigned thread_arena = N_ARENA;
...
static union {
  struct {
    ...
    struct size_class *size_class_metadata[H_MALLOC_N_ARENA];
...
#ifdef MEMTAG
static inline bool is_memtag_enabled(void) { return !ro.is_memtag_disabled; }
#endif
...
struct slab_metadata {
  ...
  // MAX_SLAB_SLOT_COUNT is currently 256, 2 extra slots are needed for
  // branchless handling of edge slots in tag_and_clear_slab_slot()
  //
  // It's intentionally placed at the end of struct to improve locality: for
  // most size classes, slot count is far lower than MAX_SLAB_SLOT_COUNT.
  u8 arm_mte_tags[129];
#endif
};
...
#if !H_MALLOC_EXTENDED_SIZE_CLASSES
...
#if SLAB_QUARANTINE && H_MALLOC_EXTENDED_SIZE_CLASSES
...
static size_t get_slots(unsigned class) { return size_class_slots[class]; }
...
static inline struct size_info get_size_info(size_t size) {
  ...
  // size <= 64 is needed for correctness and raising it to size <= 128 is an
  // optimization
  if (size <= 128) {
  ...
  size_t spacing_class_index =
      (real_size >> spacing_class_shift) - initial_spacing_multiplier;
  size_t index =
      (spacing_class_shift - 4) * 4 + special_small_sizes + spacing_class_index;
  return (struct size_info){real_size, index};
}

// alignment must be a power of 2 <= PAGE_SIZE since slabs are only page aligned
static inline struct size_info get_size_info_align(size_t size,
                                                   size_t alignment) {
...
struct __attribute__((aligned(CACHELINE_SIZE))) size_class {
  ...
  void *quarantine_random[SLAB_QUARANTINE_RANDOM_LENGTH
                          << (MAX_SLAB_SIZE_CLASS_SHIFT -
                              MIN_SLAB_SIZE_CLASS_SHIFT)];
#endif
  ...
  struct slab_metadata
      *free_slabs_quarantine[H_MALLOC_FREE_SLABS_QUARANTINE_RANDOM_LENGTH];
...
#define CLASS_REGION_SIZE (size_t)H_MALLOC_CLASS_REGION_SIZE
#define REAL_CLASS_REGION_SIZE (CLASS_REGION_SIZE * 2)
#define ARENA_SIZE (REAL_CLASS_REGION_SIZE * N_SIZE_CLASSES)
static const size_t slab_region_size = ARENA_SIZE * H_MALLOC_N_ARENA;
static_assert(PAGE_SIZE == 4096,
              "bitmap handling will need adjustment for other page sizes");

static void *get_slab(const struct size_class *c, size_t slab_size,
                      const struct slab_metadata *metadata) {
...
static struct slab_metadata *
alloc_metadata(struct size_class *c, size_t slab_size, bool non_zero_size) {
  if (unlikely(c->metadata_count >= c->metadata_allocated)) {
    ...
    size_t allocate = max(c->metadata_allocated * 2,
                          PAGE_SIZE / sizeof(struct slab_metadata));
    if (allocate > metadata_max) {
      allocate = metadata_max;
    }
    if (unlikely(memory_protect_rw_metadata(
            c->slab_info, allocate * sizeof(struct slab_metadata)))) {
      return NULL;
    }
  ...
  if (c->metadata_count_unguarded >= H_MALLOC_GUARD_SLABS_INTERVAL) {
    c->metadata_count++;
...
static void clear_quarantine_slot(struct slab_metadata *metadata,
                                  size_t index) {
  size_t bucket = index / U64_WIDTH;
  metadata->quarantine_bitmap[bucket] &= ~(1UL << (index - bucket * U64_WIDTH));
}

static bool is_quarantine_slot(const struct slab_metadata *metadata,
                               size_t index) {
  size_t bucket = index / U64_WIDTH;
  return (metadata->quarantine_bitmap[bucket] >> (index - bucket * U64_WIDTH)) &
         1UL;
}
#endif
...
static size_t get_free_slot(struct random_state *rng, size_t slots,
                            const struct slab_metadata *metadata) {
  if (H_MALLOC_SLOT_RANDOMIZE) {
    // randomize start location for linear search (uniform random choice is too
    // slow)
...
static bool has_free_slots(size_t slots, const struct slab_metadata *metadata) {
  ...
  if (slots <= U64_WIDTH * 3) {
    ...
    return metadata->bitmap[0] != ~0UL || metadata->bitmap[1] != ~0UL ||
           masked != ~0UL;
  }
  u64 masked = metadata->bitmap[3] | get_mask(slots - U64_WIDTH * 3);
  return metadata->bitmap[0] != ~0UL || metadata->bitmap[1] != ~0UL ||
         metadata->bitmap[2] != ~0UL || masked != ~0UL;
#endif
}
...
static struct slab_metadata *get_metadata(const struct size_class *c,
                                          const void *p) {
  size_t offset = (const char *)p - (const char *)c->class_region_start;
  size_t index = libdivide_u64_do(offset, &c->slab_size_divisor);
  // still caught without this check either as a read access violation or
  // "double free"
...
static void write_after_free_check(const char *p, size_t size) {
  if (!H_MALLOC_WRITE_AFTER_FREE_CHECK) {
    return;
  }
...
static void set_slab_canary_value(UNUSED struct slab_metadata *metadata,
                                  UNUSED struct random_state *rng) {
#if SLAB_CANARY
  ...
  if (unlikely(metadata->canary_value == 0)) {
    // 0 is reserved to support disabling MTE at runtime (this is required on
    // Android). When MTE is enabled, writing and reading of canaries is
    // disabled, i.e. canary remains zeroed. After MTE is disabled, canaries
    // that are set to 0 are ignored, since they wouldn't match slab's
    // metadata->canary_value. 0x100 was chosen arbitrarily, and can be encoded
    // as an immediate value on ARM by the compiler.
...
static void set_canary(UNUSED const struct slab_metadata *metadata,
                       UNUSED void *p, UNUSED size_t size) {
#if SLAB_CANARY
#ifdef HAS_ARM_MTE
  if (likely(is_memtag_enabled())) {
    return;
...
static void check_canary(UNUSED const struct slab_metadata *metadata,
                         UNUSED const void *p, UNUSED size_t size) {
#if SLAB_CANARY
#ifdef HAS_ARM_MTE
  if (likely(is_memtag_enabled())) {
    return;
...
static inline void stats_small_allocate(UNUSED struct size_class *c,
                                        UNUSED size_t size) {
#if H_MALLOC_STATS
  c->allocated += size;
  c->nmalloc++;
...
static inline void stats_small_deallocate(UNUSED struct size_class *c,
                                          UNUSED size_t size) {
#if H_MALLOC_STATS
  c->allocated -= size;
  c->ndalloc++;
...
static inline void stats_slab_allocate(UNUSED struct size_class *c,
                                       UNUSED size_t slab_size) {
#if H_MALLOC_STATS
  c->slab_allocated += slab_size;
#endif
}

static inline void stats_slab_deallocate(UNUSED struct size_class *c,
                                         UNUSED size_t slab_size) {
#if H_MALLOC_STATS
  c->slab_allocated -= slab_size;
#endif
...
#ifdef HAS_ARM_MTE
static void *tag_and_clear_slab_slot(struct slab_metadata *metadata,
                                     void *slot_ptr, size_t slot_idx,
                                     size_t slot_size) {
  // arm_mte_tags is an array of 4-bit unsigned integers stored as u8 array (MTE
  // tags are 4-bit wide)
  //
  // It stores the most recent tag for each slab slot, or 0 if the slot was
  // never used. Slab indices in arm_mte_tags array are shifted to the right by
  // 1, and size of this array is (MAX_SLAB_SLOT_COUNT + 2). This means that
  // first and last values of arm_mte_tags array are always 0, which allows to
  // handle edge slots in a branchless way when tag exclusion mask is
  // constructed.
  ...
  // current or previous tag of left neighbor or 0 if there's no left neighbor
  // or if it was never used
  ...
  // current or previous tag of right neighbor or 0 if there's no right neighbor
  // or if it was never used
...
static inline void *allocate_small(unsigned arena, size_t requested_size) {
  ...
  if (c->partial_slabs == NULL) {
    ...
    struct slab_metadata *metadata =
        alloc_metadata(c, slab_size, requested_size);
...
static void enqueue_free_slab(struct size_class *c,
                              struct slab_metadata *metadata) {
  metadata->next = NULL;

  static_assert(H_MALLOC_FREE_SLABS_QUARANTINE_RANDOM_LENGTH < (u16)-1,
                "free slabs quarantine too large");
  size_t index = get_random_u16_uniform(
      &c->rng, H_MALLOC_FREE_SLABS_QUARANTINE_RANDOM_LENGTH);
...
static inline void deallocate_small(void *p, const size_t *expected_size) {
  ...
  if (likely(!is_zero_size)) {
    ...
    if (likely(is_memtag_enabled())) {
      ...
      // metadata->arm_mte_tags is intentionally not updated, see
      // tag_and_clear_slab_slot()
      skip_zero = true;
    }
#endif

    if (H_MALLOC_ZERO_ON_FREE && !skip_zero) {
...
#if SLAB_QUARANTINE_RANDOM_LENGTH > 0
  size_t slab_quarantine_random_length = SLAB_QUARANTINE_RANDOM_LENGTH
                                         << quarantine_shift;

  size_t random_index =
      get_random_u16_uniform(&c->rng, slab_quarantine_random_length);
...
#if SLAB_QUARANTINE_QUEUE_LENGTH > 0
  size_t slab_quarantine_queue_length = SLAB_QUARANTINE_QUEUE_LENGTH
                                        << quarantine_shift;

  void *queue_substitute = c->quarantine_queue[c->quarantine_queue_index];
  c->quarantine_queue[c->quarantine_queue_index] = p;
  c->quarantine_queue_index =
      (c->quarantine_queue_index + 1) % slab_quarantine_queue_length;
...
#define MAX_REGION_TABLE_SIZE                                                  \
  (CLASS_REGION_SIZE / PAGE_SIZE / sizeof(struct region_metadata))
...
struct region_allocator {
  ...
  size_t allocated;
#endif
#if H_MALLOC_REGION_QUARANTINE_RANDOM_LENGTH
  struct quarantine_info
      quarantine_random[H_MALLOC_REGION_QUARANTINE_RANDOM_LENGTH];
#endif
#if H_MALLOC_REGION_QUARANTINE_QUEUE_LENGTH
  struct quarantine_info
      quarantine_queue[H_MALLOC_REGION_QUARANTINE_QUEUE_LENGTH];
  size_t quarantine_queue_index;
#endif
  struct random_state rng;
};

static inline void stats_large_allocate(UNUSED struct region_allocator *ra,
                                        UNUSED size_t size) {
#if H_MALLOC_STATS
  ra->allocated += size;
#endif
...
static inline void stats_large_deallocate(UNUSED struct region_allocator *ra,
                                          UNUSED size_t size) {
#if H_MALLOC_STATS
  ra->allocated -= size;
#endif
...
struct __attribute__((aligned(PAGE_SIZE))) allocator_state {
  struct size_class size_class_metadata[H_MALLOC_N_ARENA][N_SIZE_CLASSES];
  struct region_allocator region_allocator;
  // padding until next page boundary for mprotect
  struct region_metadata regions_a[MAX_REGION_TABLE_SIZE]
      __attribute__((aligned(PAGE_SIZE)));
  // padding until next page boundary for mprotect
  struct region_metadata regions_b[MAX_REGION_TABLE_SIZE]
      __attribute__((aligned(PAGE_SIZE)));
  // padding until next page boundary for mprotect
  struct slab_info_mapping slab_info_mapping[H_MALLOC_N_ARENA][N_SIZE_CLASSES];
  // padding until next page boundary for mprotect
};

static void regions_quarantine_deallocate_pages(void *p, size_t size,
                                                size_t guard_size) {
  if (!REGION_QUARANTINE || size >= H_MALLOC_REGION_QUARANTINE_SKIP_THRESHOLD) {
...
#if H_MALLOC_REGION_QUARANTINE_RANDOM_LENGTH
  size_t index = get_random_u64_uniform(
      &ra->rng, H_MALLOC_REGION_QUARANTINE_RANDOM_LENGTH);
...
#if H_MALLOC_REGION_QUARANTINE_QUEUE_LENGTH
  struct quarantine_info queue_substitute =
      ra->quarantine_queue[ra->quarantine_queue_index];
  ra->quarantine_queue[ra->quarantine_queue_index] = target;
  ra->quarantine_queue_index = (ra->quarantine_queue_index + 1) %
                               H_MALLOC_REGION_QUARANTINE_QUEUE_LENGTH;
...
static int regions_grow(void) {
  ...
  struct region_metadata *p =
      ra->regions == ro.regions[0] ? ro.regions[1] : ro.regions[0];
  ...
  memory_set_name(ra->regions, ra->total * sizeof(struct region_metadata),
                  "malloc allocator_state");
...
static void full_lock(void) {
  ...
  for (unsigned arena = 0; arena < H_MALLOC_N_ARENA; arena++) {
...
static inline bool is_init(void) { return get_slab_region_end() != NULL; }
...
COLD static void init_slow_path(void) {
  ...
  if (unlikely(sysconf(_SC_PAGESIZE) != PAGE_SIZE)) {
    fatal_error("runtime page size does not match compile-time page size which "
                "is not supported");
  }

  struct random_state *rng = allocate_pages(sizeof(struct random_state),
                                            PAGE_SIZE, true, "malloc init rng");
  ...
  size_t metadata_guard_size =
      (get_random_u64_uniform(rng, REAL_CLASS_REGION_SIZE / PAGE_SIZE) + 1) *
      PAGE_SIZE;
  ...
  if (unlikely(memory_protect_rw_metadata(
          allocator_state, offsetof(struct allocator_state, regions_a)))) {
  ...
  if (unlikely(memory_protect_rw_metadata(
          ra->regions, ra->total * sizeof(struct region_metadata)))) {
  ...
  memory_set_name(ro.slab_region_start, slab_region_size,
                  "malloc slab region gap");
  ...
  for (unsigned arena = 0; arena < H_MALLOC_N_ARENA; arena++) {
    ...
    for (unsigned class = 0; class < N_SIZE_CLASSES; class ++) {
      ...
      size_t bound =
          (REAL_CLASS_REGION_SIZE - CLASS_REGION_SIZE) / PAGE_SIZE - 1;
      size_t gap = (get_random_u64_uniform(rng, bound) + 1) * PAGE_SIZE;
      c->class_region_start = (char *)ro.slab_region_start +
                              ARENA_SIZE * arena +
                              REAL_CLASS_REGION_SIZE * class + gap;
  ...
  atomic_store_explicit(&ro.slab_region_end, slab_region_end,
  ...
  // may allocate, so wait until the allocator is initialized to avoid
  // deadlocking
...
#if H_MALLOC_SELF_INIT
// trigger early initialization to set up pthread_atfork and protect state as
// soon as possible
COLD __attribute__((constructor(101))) static void trigger_early_init(void) {
  // avoid calling init directly to skip it if this isn't the malloc
  // implementation
  h_free(h_malloc(16));
}
#endif
...
static size_t get_large_size_class(size_t size) {
  if (H_MALLOC_LARGE_SIZE_CLASSES) {
...
static size_t get_guard_size(struct random_state *state, size_t size) {
  return (get_random_u64_uniform(state, size / PAGE_SIZE /
                                            H_MALLOC_GUARD_SIZE_DIVISOR) +
          1) *
         PAGE_SIZE;
}
...
static inline void *allocate(unsigned arena, size_t size) {
  return size <= max_slab_size_class ? allocate_small(arena, size)
                                     : allocate_large(size);
}
...
static int allocate_aligned(unsigned arena, void **memptr, size_t alignment,
                            size_t size, size_t min_alignment) {
...
EXPORT void *h_calloc(size_t nmemb, size_t size) {
  ...
  if (!H_MALLOC_ZERO_ON_FREE && likely(p != NULL) && total_size &&
      total_size <= max_slab_size_class) {
    memset(p, 0, total_size - canary_size);
  }
#ifdef HAS_ARM_MTE
  // use an assert instead of adding a conditional to memset() above (freed
  // memory is always zeroed when MTE is enabled)
  static_assert(
      ZERO_ON_FREE,
      "disabling ZERO_ON_FREE reduces performance when ARM MTE is enabled");
#endif
  return p;
}
  ...
  if (old < get_slab_region_end() && old >= ro.slab_region_start) {
    ...
    if (size > max_slab_size_class) {
      ...
      if (vma_merging_reliable) {
        ...
        if (!memory_remap((char *)old + old_size, old_guard_size,
                          old_guard_size + extra)) {
...
static inline void memory_corruption_check_small(const void *p) {
  ...
  void *slab = get_slab(c, slab_size, metadata);
  size_t slot =
      libdivide_u32_do((const char *)p - (const char *)slab, &c->size_divisor);
...
EXPORT size_t h_malloc_object_size(const void *p) {
  ...
  if (p < slab_region_end && p >= ro.slab_region_start) {
    ...
    struct size_class *c =
        &ro.size_class_metadata[size_class_info.arena][class];
    ...
    void *slab = get_slab(c, slab_size, metadata);
    size_t slot = libdivide_u32_do((const char *)p - (const char *)slab,
                                   &c->size_divisor);
  ...
  for (unsigned arena = 0; arena < H_MALLOC_N_ARENA; arena++) {
    ...
    for (unsigned class = 1; class < N_SIZE_CLASSES; class ++) {
      ...
      if (size >= min_extended_size_class) {
        size_t quarantine_shift =
            clz64(size) - (63 - MAX_SLAB_SIZE_CLASS_SHIFT);
...
EXPORT struct mallinfo h_mallinfo(void) { return (struct mallinfo){0}; }
...
EXPORT size_t h_mallinfo_nbins(void) { return N_SIZE_CLASSES; }

// This internal Android API uses mallinfo in a non-standard way to implement
// malloc_info:
//
...
// fordblks: ndalloc
// (other fields are unused)
EXPORT struct mallinfo h_mallinfo_bin_info(UNUSED size_t arena,
                                           UNUSED size_t bin) {
  struct mallinfo info = {0};
...
COLD EXPORT int
h_malloc_iterate(UNUSED uintptr_t base, UNUSED size_t size,
...
COLD EXPORT int h_malloc_set_state(UNUSED void *state) { return -2; }
#endif
