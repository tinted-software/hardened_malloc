...
h_aligned_alloc(size_t alignment, size_t size);
...
__attribute__((malloc)) __attribute__((alloc_size(2)))
__attribute__((alloc_align(1))) void *
h_memalign(size_t alignment, size_t size);
#ifndef __ANDROID__
__attribute__((malloc)) __attribute__((alloc_size(1))) void *
h_valloc(size_t size);
...
int h_malloc_iterate(uintptr_t base, size_t size,
                     void (*callback)(uintptr_t ptr, size_t size, void *arg),
                     void *arg);
...
// similar to malloc_object_size, but avoiding locking so the results are much
// more limited
